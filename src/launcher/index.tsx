...
export async function createLauncher ({
  ...
  return function Laucnher () {
    ...
    return (
      ...
      >
        <div
          onClick={async () => await Neutralino.os.open(url)}
          role='button'
          class='version-icon'
          style={{
          ...
          >
            <Box flex={1}>
              <Show when={programBusy()}>
                <h3
                  style='text-shadow: 1px 1px 2px #333;color:white;margin-bottom:5px;'
                >
                  {statusText()}
                </h3>
                <Progress
            ...
            <Box>
              <ButtonGroup class='launch-button' size='xl' attached minWidth={150}>
                <Button
                  ...
                  onClick={async () => await onButtonClick().catch(fatal)}
                >
                  {_gameInstalled()
                    ? locale.get('LAUNCH')
                    : locale.get('INSTALL')}
