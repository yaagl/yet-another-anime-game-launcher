...
export async function* patchProgram(
  ...
  const renderBackend = server.id.startsWith("hkrpg")
    ? config.renderBackend
    : wine.attributes.renderBackend;
  ...
  if (renderBackend == "dxmt") {
    ...
    // Also copy winemetal.dll to the prefix.
  ...
  if (config.reshade && renderBackend == "dxmt") {
